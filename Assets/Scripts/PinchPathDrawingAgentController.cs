...
public sealed class PinchPathDrawingAgentController : MonoBehaviour
{
    ...
    [SerializeField] private Vector3 _avatarStartPosition = new Vector3(0f, -30f, 22f);
    [SerializeField] private Vector3 _avatarStartEulerAngles = new Vector3(0f, 180f, 0f);
    [SerializeField] private float _avatarStartScale = 8f;
    ...
    private void PlaceAvatarAtStart()
    {
        if (_hasPlacedAvatar || _avatar == null)
        {
            return;
        }

        _avatar.SetPositionAndRotation(_avatarStartPosition, Quaternion.Euler(_avatarStartEulerAngles));
        _avatar.localScale = Vector3.one * _avatarStartScale;
        _hasPlacedAvatar = true;
    }
